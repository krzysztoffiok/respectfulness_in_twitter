...
def ML_classification(allFeatures, train_ml, test_ml, df_ml, classification_model, language_model, folds):
    """
...
def compute_metrics_comb(dependent_variable):
    # read result files to compute metrics including SemEval2017-specific
    preds = dt.fread(f"./data/{dependent_variable}_baseline_predictions.csv").to_pandas()
    trues = dt.fread(f"./data/{dependent_variable}_baseline_trues.csv").to_pandas()
    modelColNames = preds.columns.to_list()
    modelColNames.remove("C0")
    ...
    dfmetrics = pd.DataFrame.from_dict(allmetrics)
    dfmetrics.to_csv(f"{dependent_variable}_comb_metric_results.csv")
    print(dfmetrics)


def term_frequency(train_ml, dfliwc, df, allFeatures, dependent_variable):
    foldTFfeatures = {}
    allWords = []
