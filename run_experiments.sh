...
declare -a transformer_list=('roberta-large')

#declare -a other_model_list=('fasttext' 'roberta_lstm')
declare -a other_model_list=('fasttext')
...
declare -a pretrained_model_list=('roberta-large-nli-stsb-mean-tokens')

# declare dependent variable and number of folds
dependent_variable="respect"
folds=2 ### CHANGE FOLDS
let "embed_folds=$folds - 1"

# split the input data
python3 input_data_splitter.py

# train selected models
for i in "${transformer_list[@]}"; do python3 ./model_train.py --dependent_variable=$dependent_variable --folds=$folds \
 --test_run="$i" --fine_tune; done
#for i in "${other_model_list[@]}"; do python3 ./model_train.py --dependent_variable=$dependent_variable --folds=$folds \
...
--dependent_variable=$dependent_variable --fold=$j --test_run="$i"; done; done
...
for i in "${pretrained_model_list[@]}"; do python3 embed_data.py \
 --dependent_variable=$dependent_variable --test_run="$i" --pool=True; done

# carry out the final ML predictions
for i in "${transformer_list[@]}"; do python3 ./ml_single_model_procedure.py --test_run="$i" \
 --dependent_variable=$dependent_variable --folds=$folds;done
...
for i in "${pretrained_model_list[@]}"; do python3 ./ml_single_model_procedure.py --test_run="$i" \
 --dependent_variable=$dependent_variable --folds=$folds --pooled;done
...
runtime=$((end-start))
echo $runtime
destdir=./data/experiment_total_time.txt
